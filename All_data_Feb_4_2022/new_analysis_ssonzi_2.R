...
ggplot2::ggplot(data = Masters_Research_Final_for_Modelling)+
  aes(x=Speed, y=New_Flow, stat = "identity")+geom_point()


ggplot2::ggplot(data = Masters_Research_Final_for_Modelling)+
  aes(x=Density, y=New_Flow, stat = "identity")+geom_point()


summary(flow_model_4 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = New_Flow  ~ Speed ))


summary(flow_model_5 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = New_Flow  ~ Density ))



summary(flow_model_6 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = Flow  ~ 
               I(Speed) + I(log_Speed) + I(sq_Speed) +
               I(Density) + I(log_Density) + I(inv_Density) + I(sq_Density) ))


Masters_Research_Final_for_Modelling$New_Flow =
  predict(flow_model_2, newdata = Masters_Research_Final_for_Modelling)


summary(Masters_Research_Final_for_Modelling$New_Flow)


summary(flow_model_7 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = New_Flow  ~ 
               Speed + log_Speed + Density + log_Density ))


summary(flow_model_8 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = New_Flow  ~ Speed + Density ))


summary(flow_model_9 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = New_Flow  ~ log_Speed + log_Density ))
...
summary(flow_model <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = Flow  ~ 
               Density + log_Density + sq_Density + inv_Density  ))


summary(flow_model <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = log(Flow)  ~ 
               Speed + log(Speed) + (Speed^2) +
               Density + log(Density) + (Density^2)  ))


summary(flow_model <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = log(Flow)  ~ 
               log(Speed) + I((log(Speed))^2) +
               log(Density) + I((log(Density))^2)  ))
...
summary(flow_model_1 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = Flow  ~ 0 + Speed + I(Speed^2) + 
               Density + I(Density^2) ))


summary(flow_model_1 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = Flow  ~ 0 + I(log(Speed)) + I(log(Speed^2)) + 
               I(log(Density)) + I(log(Density^2))  ))


summary(flow_model_1 <- 
          lm(data = Masters_Research_Final_for_Modelling, 
             formula = Flow  ~ 0 + log_Speed + I(log(sq_Speed)) + 
               log_Density + I(log(sq_Density))  ))
...
ggplot(data = Masters_Research_Final_for_Modelling)+
  aes(x=Speed, y=new_flow)+
  geom_line(stat = "identity")


ggplot(data = Masters_Research_Final_for_Modelling)+
  aes(x=Density, y=new_flow)+
  geom_line(stat = "identity")
